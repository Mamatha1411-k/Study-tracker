...
# --- TIMEZONE FIX FOR INDIAN STANDARD TIME (IST) ---
# This forces the app to match your exact local phone clock
def get_local_time():
...
# Get accurate local timestamp
local_now = get_local_time()
current_date_str = local_now.strftime("%d %B %Y")
current_time_str = local_now.strftime("%I:%M %p")

st.caption(f"📅 Date: {current_date_str} | ⏰ System Time (IST): {current_time_str}")
...
with left_panel:
    ...
    if not st.session_state.timer_running:
        if st.button("🟢 START SESSION", use_container_width=True):
            ...
            st.session_state.start_clock_time = get_local_time()
            st.rerun()
        ...
        if st.button("🔴 STOP & SAVE SESSION", use_container_width=True):
            stop_clock_time = get_local_time()
            ...
            if st.session_state.streak == 0 and final_delta_hours > 0.01:
